...
void set_blocks(fight_t *fight)
{
    ...
    for (int i = 0; i < 20; i++) {
        ...
        sfRectangleShape_setPosition(fight->solid[i],
        (sfVector2f){100 * (1 + rand() % 18),
        100 * (randy == 6 ? 6 + rand() % 6 : randy)});
    }
}
...
fight_t *init_fight(void)
{
    ...
    sfRectangleShape_setPosition(fight->solid[0],
    (sfVector2f){100 * (7), 100 * (9)});
    fight->test = 0;
    fight->dir = malloc(sizeof(sfVector2f) * DMG);
    fight->clock_zigzag = create_clock();
    fight->clock_square = create_clock();
...
{
    for (int i = 0; i < DMG; i++) {
        destroy_clock(fight->time[i]);
        sfRectangleShape_destroy(fight->dmg[i]);
    }
    for (int i = 0; i < 20; i++)
        sfRectangleShape_destroy(fight->solid[i]);
    destroy_clock(fight->clock_zigzag);
    destroy_clock(fight->clock_square);
